...
|---|---|---|---|
| 1 | [2] 채널 패널 "보고서 출력" 버튼 | 단일 채널 컨텍스트 | 현 채널 1개 선택됨 |
...
| 5 | [11] 실시간 측정 중지(F7) → 자동 옵션 (선택 — v18.1+) | 측정 직후 | 측정 중이던 채널 전체 |
...
### 2-2. 섹션 1. 적용 표준 / 검사 대상

| 필드 | 출처 | 비고 |
|---|---|---|
| 적용 표준 | `TB_INSPECTION_ITEM.ApplicableStandard` | [6] 검사 대상 등록 시 입력. dropdown 8종 + 사용자 입력 |
| 검사 대상 | `TB_INSPECTION_ITEM.ItemName` + 형상 · 두께 · 소재 요약 | 자동 |
| 측정 위치 | `TB_CHANNEL_PLACEMENT.PositionTheta` + `PositionZ` | [4-3] 채널 배치 시 입력 (mm) |
...
### 4-1. React 컴포넌트 (프로토타입)

`window.ReportExportDialog` (Screens.jsx)

```typescript
interface ReportExportDialogProps {
  targetId: string;                    // 검사 대상 ID (예: "PIPE-A-204")
  ...
  onExport: (payload: {
    ...
    includeSign: boolean;
    standard: string;                   // 적용 표준 (readonly, MOCK.targetStandards에서)
  }) => void;
}
```

...
- **[C-RPT-02] 스캔 결과 보고서** = [C-PRJ-03] 스캔 pane에서 **측정(ruler·2점 클릭 또는 드래그 영역) → 캡처(⧉)** 한 결과가 쌓이는 소재 보관함 **+ 보고서 생성**. 캡처는 측정 후에만 활성 · 대상 = pane 1개 · **캡처 1장 = 채널 1개**(엔코더로 단일 채널이 B/C/D-scan을 그림) · **캡처 시점 설정값 7종 동결**(교정 세션 ID·글로벌 dB·게이트·**TCG/DAC**·음속·Probe Delay·굴절각θ — [C-PRJ-06]에서 게이트·Gain·TCG 사후 조정이 가능하므로 캡처 시점 값 박제 필수) · 좌표는 **모재 전체 기준·정수** · 좌표 라벨은 스캔 유형별 치환(C `X/Y` · B `X/깊이` · D `Y/깊이`).

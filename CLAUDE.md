...
- 흐름: **[C-PRJ-01] 시작 → [C-PRJ-02] 종합 결과 화면·DAQ 장비 목록 → [C-PRJ-03] DAQ 결과 화면(측정)**. 측정 부속 = **[C-PRJ-03] 교정-사이드바 / [C-PRJ-03] 교정-파형 영역** 2문서. 교정 = **모달 아님 · 우측 436px 사이드바** + **탭 없이 ①~⑤ 세로 나열**(탐촉자 선택 → ①Pulser Control → ②영점 → ③음속 → ④경사각 → ⑤감도) · Gate는 파형 영역 드래그+인라인 바 · **[C-PRJ-03-02] Gain 설정·[C-PRJ-03-03] TCG 설정 모달은 교정으로 병합·폐기**.
- 설정(메뉴바 '설정' · 좌 트리/nav): **[C-SET-01]** DAQ 장비 설정(트리 구조(공통) + DAQ 3분할(Bridge PC 설정/DAQ 장비 정보/MQTT 설정) + 탐촉자/홀더/홀더-장착 탐촉자/I/O 장비 = **8문서**) · **[C-SET-02]** 모재 관리 · **[C-SET-03]** 모재 프리셋 관리 · **[C-SET-04]** 검사 시작 준비(맵핑) · **[C-SET-05]** 기본 설정 · **[C-SET-05]** 모니터링 서버 설정(같은 화면·좌 nav 2항목 → **문서 2개·ID 공유** · SET 번호는 01~05, 06 없음).

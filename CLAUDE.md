...
- 재교정 = **[C-CAL-01-01]** 탐촉자 재교정(교정 본체는 [C-PRJ-05]). 보고서(메뉴바 **'보고서'** 대분류 · 2026-08-25 신설) = **[C-RPT-01]** 측정 보고서(Ctrl+P) · **[C-RPT-02]** 스캔 보고서 · **[C-RPT-03]** 캡처보드 관리. 알림 NTF-01 · 로그 LOG-01.
- **캡처보드([C-RPT-03])** = [C-PRJ-03] 스캔 pane에서 **측정(ruler·2점 클릭) → 캡처(⧉)** 한 결과가 쌓이는 보고서 소재 보관함. 캡처는 측정 후에만 활성 · 대상 = pane 1개 · **캡처 시점 설정값 5종 동결**(교정 세션 ID·글로벌 dB·게이트·음속·Probe Delay) · 좌표 라벨은 스캔 유형별 치환(C `X/Y` · B `X/깊이` · D `Y/깊이`).

...
- 흐름: **[C-PRJ-01] 시작 → [C-PRJ-02] 종합 결과 화면·DAQ 장비 목록 → [C-PRJ-03] DAQ 결과 화면(측정)**. 측정 부속 = **[C-PRJ-03-01]** 교정(**사이드바 / 파형 영역** 2문서). 교정 = **모달 아님 · 결과 화면 우측 440px 사이드바** + 탭 4개(**음속·딜레이·경사각·감도**) · Gate는 탭 아님(파형 영역 드래그+인라인 바) · **[C-PRJ-03-02] Gain 설정·[C-PRJ-03-03] TCG 설정 모달은 교정으로 병합·폐기**.

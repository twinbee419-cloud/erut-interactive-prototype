...
- 설정(메뉴바 '설정' · 좌 트리/nav): **[C-SET-01]** 기본 설정 / 모니터링 서버 설정(같은 화면·좌 nav 2항목 → **문서 2개·ID 공유**) · **[C-SET-02]** DAQ 장비 설정(트리 구조(공통) + DAQ 3분할(Bridge PC 설정/DAQ 장비 정보/MQTT 설정) + 탐촉자/홀더/홀더-장착 탐촉자/I/O 장비 = **8문서**) · **[C-SET-03]** 모재 관리 · **[C-SET-04]** 모재 프리셋 관리 · **[C-SET-05]** 홀더 맵핑 · **[C-SET-06]** 스캔 계획. (구 [C-SET-04] 검사 시작 준비가 홀더 맵핑 + 스캔 계획으로 분리됨)

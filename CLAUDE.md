...
- 재교정 = **[C-CAL-01-01]** 탐촉자 재교정(교정 본체는 [C-PRJ-05]). 보고서(메뉴바 **'보고서'** 대분류) = **[C-RPT-01]** 측정 보고서(Ctrl+P) · **[C-RPT-02]** 스캔 결과 보고서 · **[C-RPT-03]** 스캔 결과 보고서 이력. 알림 = **[C-NTF-01]** 알림-패널(종 아이콘 · 우측 340px · 최근 7일 요약) / 알림-페이지(문서 탭 · 전체 이력 테이블) **2문서·ID 공유** · 로그 LOG-01.

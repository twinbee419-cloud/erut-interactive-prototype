...
Claude Code 작업 지침. **ERUT** (Eroun Realtime Ultrasonic Testing) — 자석 부착 로봇 기반 초음파 검사 시스템. 윈도우 프로그램명 = **ERUT Client**. 현재 **코드 없는 사전 개발 단계 — 기획/UI mockup 작업 중**. **프로젝트는 고정형·스캔형 통합**(유형 구분 폐지 · 2026-07-14) — ERUT Client가 **고정형(A-scan) + 스캔형(C-scan) 모두** 다룸.
...
## 저장소 구조 · 산출물 (빌드 없음)
- **두 개의 산출물**을 구분: ① **메인 기획(mockup)** = `ERUT_ServiceFlow_FixedProbe.html` — 1920×1080 화면들을 `.slide`로 담은 **정적 슬라이드 문서**(SSOT 기획, `SLIDE N` 주석으로 구획). 스캔형은 `ERUT_ServiceFlow_Crawler.html`. ② **프로토타입** = `design-system/project/ui_kits/erut-windows/` — 동작하는 React 프로토타입.
- **빌드 스텝 없음**: 프로토타입은 `index.html`이 CDN UMD(React 18.3.1) + Babel standalone로 `.jsx`를 **브라우저에서 직접 트랜스파일**. 화면 로직 대부분은 `Screens.jsx`(≈4100줄), 창 chrome은 `Chrome.jsx`, 폼 컨트롤은 `Controls.jsx`, 아이콘은 `Icons.jsx`. 실행 = 파일을 브라우저로 열기(번들러·npm 불필요).
- **배포**: `vercel.json`이 `/` → `design-system/.../erut-windows/index.html`로 rewrite. **git push = Vercel 자동 배포**(git-tracked 변경만 반영).
- **git 추적 vs 로컬 전용**: 추적 = `design-system/` · `dev_handoff/` · `CLAUDE.md` · `index.html` · `vercel.json`. **gitignore(로컬 전용)** = 메인 `*.html`(FixedProbe·Crawler 등) · `archive/` · `*.pdf/docx/fig`. → 메인 기획 mockup을 고쳐도 배포엔 안 나타남; 배포에 반영하려면 **프로토타입(JSX)에 sync**해야 함.
- **sync 방향(단방향)**: 메인 기획 → 프로토타입은 **즉시 sync 의무**. 역방향(프로토타입 → 기획)은 **명시 지시 시에만**.
- **개발 인계**: `dev_handoff/*.md` = 개발팀용 기능 사양(PRF 자동계산·탐촉자 등록·알림·DAQ 자동정보·Store&Forward 등). 화면 변경 시 관련 spec 동기화.
...
12. **현재형**으로만 — 폐기·교체 기능 서술 삭제 (단, 현재 설계 이유가 되는 도메인 rationale은 현재형 보존, 예: "고정형 A-scan은 단일 위치라 좌표 미등록 · 스캔형 C-scan은 엔코더·좌표 등록").
...
## NDT 도메인 (통합 = 고정형 A-scan + 스캔형 C-scan)
- 범위 = **고정형(A-scan) + 스캔형(C-scan) 통합**. 고정형 = 수직 탐촉자·두께/감육·단일 위치(좌표 미등록). 스캔형 = 엔코더·좌표(X/Y·곡률·SOI)·C-scan·글로벌 dB 사후조정. **스캔형 도메인 1순위 = `지사장님_교육자료/상용 소프트웨어(FastView) 기능 정리본.md`.** (아래 A-scan 규칙은 고정형 기준 — 스캔형은 FastView 정리본 참조)
...
## 화면 · 흐름 (현재 mockup 기준)
- 메인 기획 mockup: `ERUT_ServiceFlow_FixedProbe.html`(A-scan) · `ERUT_ServiceFlow_Crawler.html`(C-scan). **통합 프로젝트라 두 유형이 한 프로그램(ERUT Client) — 유형 구분(고정/스캔) 폐지.** 화면설계서 SSOT = Notion DB([C-...] 화면ID).
- 흐름: **[0] 프로젝트 선택 → [1] 메인(DAQ 목록) → [2] 장비 상세(검사대상 + 64ch 그리드) → [11] 실시간**. 설정 분기: **[4]** 장비연결(DAQ·MQTT) · **[4-3-1]** 채널 설정 · **[6]** 검사대상 관리 · **[8]** 설정 · **[18]** 채널 보고서.
- **측정 제어 = DAQ 단위**([2] 배너 + 툴바 활성 DAQ). 탐촉자 개별 start/stop 불가(64ch 동시 PRF).
...
- 상세 아키텍처·BLService 인터페이스·DB 스키마·C-SCAN 포맷·파일시스템: **기획서 PDF + `dev_handoff/`** 참조. (C-SCAN·좌표정합·3D는 **스캔형 = 통합으로 범위 내** · 스캔형 상세는 FastView 정리본)

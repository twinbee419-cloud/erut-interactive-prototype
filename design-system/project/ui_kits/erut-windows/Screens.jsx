...
window.MainScreen = function MainScreen({ onPickTarget, onAddTarget, onAddDevice, onOpenDevice, onChangeProject }) {
  ...
  function renderDeviceCard(d) {
    ...
    const cardStyle = isMeasuring
      ? { border: "1px solid var(--border-emphasis)", background: "linear-gradient(rgba(34,133,239,0.06),rgba(34,133,239,0.06)), var(--surface-subtle-2)", padding: "12px 14px" }
      : isOffline
      ...
      <div key={d.id} style={cardStyle}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-end" }}>
            ...
            {isMeasuring && <MiniPill tone="emphasis" ledColor="green">측정 중</MiniPill>}
            {d.state === "idle"     && <MiniPill tone="neutral">대기</MiniPill>}
            {isOffline              && <MiniPill tone="error">오프라인</MiniPill>}
          </div>
        </div>
...
// 검사 대상 카드 (메인 .inspect-card 매칭 — thumbnail + name/desc + 결함/세션/판정)
function InspectCard({ target, featured, judgeColor, onClick }) {
  return (
    <div
      onClick={onClick}
      style={{
        border: featured ? "1px solid var(--border-emphasis)" : "1px solid var(--border-medium)",
        background: featured
          ? "linear-gradient(rgba(34,133,239,0.05),rgba(34,133,239,0.05)), var(--surface-base)"
          : "var(--surface-base)",
        padding: "16px 18px",
        display: "flex",
    ...
    >
      <div style={{ height: 130, background: "var(--surface-subtle-1)", border: "1px solid var(--border-low)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <TargetThumb kind={target.thumb}/>
      </div>
      <div>
        <div style={{ font: "700 14px/1.2 var(--font-kr)", letterSpacing: ".02em", color: "var(--content-high)" }}>{target.name}</div>
        <div style={{ font: "400 11px/1.4 var(--font-kr)", letterSpacing: ".02em", color: "var(--content-low)", marginTop: 4 }}>{target.desc}</div>
      </div>
      <div style={{ display: "flex", gap: 16, marginTop: "auto", paddingTop: 10, borderTop: "1px solid var(--border-low)" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          <span style={{ font: "700 16px/1 var(--font-kr)", letterSpacing: ".02em", color: target.defects > 0 ? "var(--system-error)" : "var(--content-high)" }}>{target.defects}</span>
          <span style={{ font: "400 11px/1 var(--font-kr)", letterSpacing: ".02em", color: "var(--content-low)" }}>결함</span>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          <span style={{ font: "700 16px/1 var(--font-kr)", letterSpacing: ".02em", color: "var(--content-high)" }}>{target.sessions}</span>
          <span style={{ font: "400 11px/1 var(--font-kr)", letterSpacing: ".02em", color: "var(--content-low)" }}>세션</span>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 2, marginLeft: "auto", alignItems: "flex-end" }}>
          <span style={{ font: "700 13px/1 var(--font-kr)", letterSpacing: ".02em", color: judgeColor(target.judgeT) }}>{target.judge}</span>
          <span style={{ font: "400 11px/1 var(--font-kr)", letterSpacing: ".02em", color: "var(--content-low)" }}>판정</span>
        </div>
      </div>
    </div>
  );
...
// =================== Screen 5 · [1-1] INSPECTION DASHBOARD (v6.0) ===================
window.InspectionDashboard = function InspectionDashboard({ targetId, onBack, onOpenDevice, onStartInspection, onOpenManage }) {
  const target = window.MOCK.targets.find(t => t.id === targetId) || window.MOCK.targets[0];
  const [layers, setLayers] = $s({ all: true, defect: true, weld: true, sensor: true });
  const [hover, setHover]   = $s(null); // sensor id
  const [hoverPos, setHoverPos] = $s({ x: 0, y: 0 });

  // 2D 전개도 좌표 변환: θ 0~360 → y, Z 0~6000 → x. Plot rect: 35~960, 30~390
  const PLOT = { x0: 60, x1: 960, y0: 40, y1: 400, zMax: 6000 };
  const toX = (z)  => PLOT.x0 + (z / PLOT.zMax) * (PLOT.x1 - PLOT.x0);
  const toY = (th) => PLOT.y0 + (th / 360) * (PLOT.y1 - PLOT.y0);

  const stateColor = (s) => s === "warn" ? "var(--system-caution)" : s === "err" ? "var(--system-error)" : "var(--system-success)";

  return (
    <div className="erut-page-enter" style={{ padding: "16px 30px", height: "100%", display: "flex", flexDirection: "column", gap: 12 }}>
      {/* 헤더 + breadcrumb */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          <div className="erut-bread" style={{ marginBottom: 6 }}>
            <window.Button variant="subtle" size="sm" onClick={onBack}>← 메인</window.Button>
            <span style={{ marginLeft: 8 }}>{window.MOCK.project.name}</span>
            <span className="erut-bread__sep">›</span>
            <span style={{ color: "var(--content-medium)" }}>검사 대상</span>
            <span className="erut-bread__sep">›</span>
            <span className="erut-bread__cur">{target.name}</span>
          </div>
          <div style={{ font: "700 24px/1.2 var(--font-kr)", letterSpacing: ".02em", color: "var(--content-high)" }}>{target.name}</div>
          <div style={{ font: "400 13px/1.4 var(--font-kr)", letterSpacing: ".02em", color: "var(--content-low)", marginTop: 4 }}>{target.desc} · 고온 스팀 380°C / 4.2 MPa · KS B 0817</div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <window.Button variant="default" size="m" onClick={onOpenManage}>정보 수정 → [6]</window.Button>
          <window.Button variant="emphasis" size="m" onClick={onStartInspection}>+ 새 검사 시작</window.Button>
        </div>
      </div>

      {/* 메인: 도면 + 우측 패널 */}
      <div style={{ display: "grid", gridTemplateColumns: "1020px 1fr", gap: 16, minHeight: 0, flex: 1 }}>
        {/* 좌: 도면 2D 전개도 */}
        <div style={{ background: "var(--surface-base)", border: "1px solid var(--border-medium)", padding: 14, display: "flex", flexDirection: "column", gap: 10, position: "relative" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ font: "700 12px/1 var(--font-kr)", letterSpacing: ".08em", color: "var(--content-low)", textTransform: "uppercase" }}>도면 · 2D 전개도 (θ × Z)</div>
            <div style={{ display: "flex", gap: 4 }}>
              <window.Button variant={layers.all ? "active" : "default"} size="sm" onClick={() => setLayers(l => ({ ...l, all: !l.all }))}>전체보기</window.Button>
              <window.Button variant={layers.defect ? "active" : "default"} size="sm" onClick={() => setLayers(l => ({ ...l, defect: !l.defect }))}>결함</window.Button>
              <window.Button variant={layers.weld ? "active" : "default"} size="sm" onClick={() => setLayers(l => ({ ...l, weld: !l.weld }))}>용접선</window.Button>
              <window.Button variant={layers.sensor ? "active" : "default"} size="sm" onClick={() => setLayers(l => ({ ...l, sensor: !l.sensor }))}>센서 위치</window.Button>
            </div>
          </div>

          <div style={{ background: "var(--surface-subtle-1)", border: "1px solid var(--border-low)", height: 440, position: "relative" }}>
            <svg viewBox="0 0 1000 440" style={{ width: "100%", height: "100%" }} preserveAspectRatio="xMidYMid meet">
              {/* 플롯 영역 */}
              <rect x={PLOT.x0} y={PLOT.y0} width={PLOT.x1-PLOT.x0} height={PLOT.y1-PLOT.y0} fill="var(--surface-base)" stroke="var(--border-medium)" strokeWidth="1"/>
              {/* θ 가이드라인 */}
              {[90, 180, 270].map(t => (
                <line key={t} x1={PLOT.x0} y1={toY(t)} x2={PLOT.x1} y2={toY(t)} stroke="var(--border-low)" strokeWidth="0.5" strokeDasharray="2,3"/>
              ))}
              {/* θ 라벨 */}
              {[0, 90, 180, 270, 360].map(t => (
                <text key={t} x={PLOT.x0-8} y={toY(t)+4} textAnchor="end" fontSize="10" fill="var(--content-low)" fontFamily="NanumSquare">{t}°</text>
              ))}
              <text x={20} y={(PLOT.y0+PLOT.y1)/2} textAnchor="middle" fontSize="10" fill="var(--content-low)" fontFamily="NanumSquare" transform={`rotate(-90 20 ${(PLOT.y0+PLOT.y1)/2})`}>θ (둘레)</text>
              {/* Z 라벨 */}
              <text x={PLOT.x0} y={PLOT.y1+18} fontSize="10" fill="var(--content-low)" fontFamily="NanumSquare">0 mm</text>
              <text x={(PLOT.x0+PLOT.x1)/2} y={PLOT.y1+18} textAnchor="middle" fontSize="10" fill="var(--content-low)" fontFamily="NanumSquare">Z (길이)</text>
              <text x={PLOT.x1} y={PLOT.y1+18} textAnchor="end" fontSize="10" fill="var(--content-low)" fontFamily="NanumSquare">6,000 mm</text>

              {/* 용접선 */}
              {layers.weld && window.MOCK.welds.map(w => (
                <g key={w.id}>
                  <line x1={toX(w.z)} y1={PLOT.y0} x2={toX(w.z)} y2={PLOT.y1} stroke="var(--brand-primary)" strokeWidth="1" strokeDasharray="4,3"/>
                  <text x={toX(w.z)} y={PLOT.y0-6} textAnchor="middle" fontSize="10" fill="var(--brand-primary)" fontFamily="NanumSquare" fontWeight="700">{w.id}</text>
                </g>
              ))}

              {/* 결함 */}
              {layers.defect && window.MOCK.defects.map(d => {
                const r = d.type === "Critical" ? 11 : d.type === "Major" ? 8 : 6;
                const c = d.type === "Critical" ? "var(--system-error)" : d.type === "Major" ? "var(--system-caution)" : "var(--system-info)";
                return (
                  <g key={d.id}>
                    <circle cx={toX(d.z)} cy={toY(d.theta)} r={r} fill={c} stroke="var(--surface-base)" strokeWidth="2"/>
                    <text x={toX(d.z)} y={toY(d.theta)+4} textAnchor="middle" fontSize="11" fill="white" fontWeight="700" fontFamily="NanumSquare">{d.id}</text>
                  </g>
                );
              })}

              {/* 센서 */}
              {layers.sensor && window.MOCK.sensors.map(s => {
                const cx = toX(s.z);
                const cy = toY(s.theta);
                const isWarn = s.state === "warn";
                return (
                  <g key={s.id}
                     style={{ cursor: "pointer" }}
                     onMouseEnter={(e) => { setHover(s.id); }}
                     onMouseMove={(e) => {
                       const rect = e.currentTarget.ownerSVGElement.getBoundingClientRect();
                       const x = ((e.clientX - rect.left) / rect.width) * 1000;
                       const y = ((e.clientY - rect.top) / rect.height) * 440;
                       setHoverPos({ x: x + 12, y: y + 12 });
                     }}
                     onMouseLeave={() => setHover(null)}
                     onDoubleClick={() => onOpenDevice && onOpenDevice(s.id)}>
                    <circle
                      className={isWarn ? "erut-sensor-breathe" : undefined}
                      cx={cx} cy={cy} r="5.5"
                      fill={stateColor(s.state)}
                      stroke="var(--surface-base)" strokeWidth="1.5"
                    />
                    <text x={cx+10} y={cy+4} fontSize="10" fontWeight="700" fontFamily="NanumSquare" fill={isWarn ? "var(--system-caution)" : "var(--content-high)"}>{s.id}{isWarn ? " ⚠" : ""}</text>
                  </g>
                );
              })}
              ...
              {hover && (() => {
                const s = window.MOCK.sensors.find(x => x.id === hover);
                if (!s) return null;
                const cx = toX(s.z), cy = toY(s.theta);
                const tipW = 230, tipH = 132;
                let tx = cx + 16, ty = cy + 8;
                if (tx + tipW > 990) tx = cx - tipW - 16;
                if (ty + tipH > 430) ty = cy - tipH - 8;
                return (
                  <foreignObject x={tx} y={ty} width={tipW} height={tipH} style={{ overflow: "visible" }}>
                    <div className="erut-tooltip" style={{ position: "static", width: tipW-2 }}>
                      <div className="erut-tooltip__title">
                        <span>{s.id} · {s.state === "warn" ? "경고" : "정상"}</span>
                        {s.state === "warn" && <span className="warn">⚠</span>}
                      </div>
                      <div className="erut-tooltip__row"><span className="label">마지막 측정</span><span className={"value" + (s.state === "warn" ? " warn" : "")}>{s.thickness.toFixed(2)} mm</span></div>
                      <div className="erut-tooltip__row"><span className="label">진폭 / ToF</span><span className="value">{s.amp}% · {s.tof} μs</span></div>
                      <div className="erut-tooltip__row"><span className="label">시각</span><span className="value">{s.age}</span></div>
                      <div className="erut-tooltip__sparklabel">7일 두께 트렌드</div>
                      <window.Sparkline values={s.trend} color={s.state === "warn" ? "var(--system-caution)" : "var(--brand-primary)"} width={210} height={22}/>
                      <div className="erut-tooltip__hint">더블클릭 → [2] 장비 상세</div>
                    </div>
                  </foreignObject>
                );
              })()}
          ...
          {/* 범례 */}
          <div style={{ display: "flex", gap: 14, font: "400 11px/1 var(--font-kr)", letterSpacing: ".02em", color: "var(--content-low)", flexWrap: "wrap" }}>
            <span><span style={{ display: "inline-block", width: 8, height: 8, background: "var(--system-error)", borderRadius: "50%", marginRight: 4, verticalAlign: "middle" }}/>Critical 1</span>
            <span><span style={{ display: "inline-block", width: 8, height: 8, background: "var(--system-caution)", borderRadius: "50%", marginRight: 4, verticalAlign: "middle" }}/>Major 1</span>
            <span><span style={{ display: "inline-block", width: 8, height: 8, background: "var(--system-info)", borderRadius: "50%", marginRight: 4, verticalAlign: "middle" }}/>Minor 1</span>
            <span style={{ borderLeft: "1px solid var(--border-medium)", paddingLeft: 12, marginLeft: 4 }}><span style={{ display: "inline-block", width: 8, height: 8, background: "var(--system-success)", borderRadius: "50%", marginRight: 4, verticalAlign: "middle" }}/>정상 7</span>
            <span><span style={{ display: "inline-block", width: 8, height: 8, background: "var(--system-caution)", borderRadius: "50%", marginRight: 4, verticalAlign: "middle" }}/>경고 1</span>
            <span style={{ marginLeft: "auto", color: "var(--brand-primary)", fontWeight: 700 }}>--- 용접선 W01·W02·W03</span>
          </div>
        </div>

        {/* 우: 정보 + 모니터링 + 통계 */}
        <div style={{ display: "flex", flexDirection: "column", gap: 10, minHeight: 0, overflow: "auto" }}>
          <div style={{ background: "var(--surface-base)", border: "1px solid var(--border-medium)", padding: "12px 16px" }}>
            <div style={{ font: "700 12px/1 var(--font-kr)", letterSpacing: ".08em", color: "var(--content-low)", textTransform: "uppercase", marginBottom: 8 }}>검사 대상 정보</div>
            <div style={{ display: "grid", gridTemplateColumns: "110px 1fr", gap: "4px 12px", font: "400 12px/1.45 var(--font-kr)", letterSpacing: ".02em" }}>
              <div style={{ color: "var(--content-low)" }}>형태</div><div style={{ color: "var(--content-high)" }}>배관 (Cylindrical)</div>
              <div style={{ color: "var(--content-low)" }}>소재</div><div style={{ color: "var(--content-high)" }}>탄소강 (S355) · 음속 5,920 m/s</div>
              <div style={{ color: "var(--content-low)" }}>외경 · 두께</div><div style={{ color: "var(--content-high)" }}>∅ 300 mm · 10 mm</div>
              <div style={{ color: "var(--content-low)" }}>길이</div><div style={{ color: "var(--content-high)" }}>6,000 mm</div>
              <div style={{ color: "var(--content-low)" }}>유체</div><div style={{ color: "var(--content-high)" }}>고온 스팀 · 380°C / 4.2 MPa</div>
              <div style={{ color: "var(--content-low)" }}>적용 표준</div><div style={{ color: "var(--content-high)" }}>KS B 0817</div>
              <div style={{ color: "var(--content-low)" }}>허용 감육</div><div style={{ color: "var(--content-high)" }}>2.0 mm (공칭 대비 20%)</div>
              <div style={{ color: "var(--content-low)" }}>부착 센서</div><div style={{ color: "var(--content-high)" }}>8 / 64 채널 (ch01~ch08)</div>
            </div>
          </div>

          <div className="erut-mon-widget">
            <div className="erut-mon-widget__head">
              <div className="erut-mon-widget__title">상시 모니터링</div>
              <div className="erut-mon-widget__tag">v6.0 NEW</div>
            </div>
            <div className="erut-mon-widget__grid">
              <div className="erut-mon-widget__cell"><div className="l">활성 채널</div><div className="v"><span style={{ color: "var(--system-success)" }}>●</span> 8 / 64</div></div>
              <div className="erut-mon-widget__cell"><div className="l">측정 주기</div><div className="v">10 분</div></div>
              <div className="erut-mon-widget__cell"><div className="l">마지막 측정</div><div className="v">1 분 전 · 평균 9.8 mm</div></div>
              <div className="erut-mon-widget__cell"><div className="l">임계값 경고</div><div className="v warn">⚠ 1건 (ch07)</div></div>
            </div>
          </div>

          <div style={{ background: "var(--surface-base)", border: "1px solid var(--border-medium)", padding: "12px 16px" }}>
            <div style={{ font: "700 12px/1 var(--font-kr)", letterSpacing: ".08em", color: "var(--content-low)", textTransform: "uppercase", marginBottom: 8 }}>검사 통계</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 6 }}>
              {[
                { v: "8", l: "총 세션", c: "var(--content-high)" },
                { v: "3", l: "결함 (C1·M1·m1)", c: "var(--system-error)" },
                { v: "05/21", l: "최종 검사", c: "var(--content-high)" },
                { v: "조건부", l: "최종 판정", c: "var(--system-caution)" },
              ].map((b, i) => (
                <div key={i} style={{ padding: "8px 6px", background: "var(--surface-subtle-2)", border: "1px solid var(--border-low)", textAlign: "center" }}>
                  <div style={{ font: "700 18px/1 var(--font-kr)", color: b.c }}>{b.v}</div>
                  <div style={{ font: "400 10px/1.2 var(--font-kr)", color: "var(--content-low)", marginTop: 3 }}>{b.l}</div>
                </div>
              ))}
            </div>
            <div style={{ marginTop: 8, padding: "8px 12px", background: "linear-gradient(rgba(255,146,0,0.05),rgba(255,146,0,0.05)), var(--surface-subtle-2)", borderLeft: "3px solid var(--system-caution)", font: "400 11px/1.4 var(--font-kr)", letterSpacing: ".02em", color: "var(--content-medium)" }}>
              <strong style={{ color: "var(--system-caution)", fontWeight: 700 }}>다음 검사 예정: 2026-06-21</strong> · Critical 결함 1건 보수 후 재검사 필요
            </div>
          </div>
        </div>
      </div>
      ...
      {/* 하단: 세션 이력 */}
      <div style={{ background: "var(--surface-base)", border: "1px solid var(--border-medium)" }}>
        <div style={{ display: "grid", gridTemplateColumns: "130px 160px 1fr 80px 90px 200px", background: "var(--surface-subtle-1)", borderBottom: "1px solid var(--border-low)", font: "700 11px/1 var(--font-kr)", letterSpacing: ".02em", color: "var(--content-high)" }}>
          <div style={{ padding: "8px 12px" }}>세션 ID</div>
          <div style={{ padding: "8px 12px" }}>일자</div>
          <div style={{ padding: "8px 12px" }}>검사자</div>
          <div style={{ padding: "8px 12px" }}>결함</div>
          <div style={{ padding: "8px 12px" }}>판정</div>
          <div style={{ padding: "8px 12px" }}>액션</div>
        </div>
        {window.MOCK.sessions.map((s, i) => (
          <div key={s.id} style={{ display: "grid", gridTemplateColumns: "130px 160px 1fr 80px 90px 200px", borderBottom: i < window.MOCK.sessions.length-1 ? "1px solid var(--border-low)" : "none", font: "400 12px/1 var(--font-kr)", alignItems: "center" }}>
            <div style={{ padding: "8px 12px", fontFamily: "'Consolas', monospace", color: i === 0 ? "var(--content-emphasis)" : "var(--content-medium)", fontWeight: 700 }}>{s.id}</div>
            <div style={{ padding: "8px 12px", color: "var(--content-high)" }}>{s.date}</div>
            <div style={{ padding: "8px 12px", color: "var(--content-high)" }}>{s.inspector}</div>
            <div style={{ padding: "8px 12px", color: s.defects > 0 ? "var(--system-error)" : "var(--system-success)", fontWeight: 700 }}>{s.defects}</div>
            <div style={{ padding: "8px 12px", color: s.judgeT === "ok" ? "var(--system-success)" : "var(--system-caution)", fontWeight: 700 }}>{s.judge}</div>
            <div style={{ padding: "8px 12px", display: "flex", gap: 4 }}>
              <window.Button variant="default" size="sm">데이터 열기</window.Button>
              <window.Button variant="subtle" size="sm">보고서</window.Button>
            </div>
          </div>
        ))}
      </div>
...
// MC보드 메타 정보 패널 + 센서 그리드 (32 셀, 스타일·데이터 보존) + 메인 매칭 사이드패널.
// Layout matches ServiceFlow_Analysis SLIDE 7 [2] 장비 상세 (v6.0)
// — 단, 7일 트렌드 sparkline은 제외 (윈도우 프로그램은 현장 검사 전용, 상시 모니터링은 웹 서비스)

window.AddSensorModal = function AddSensorModal({ onClose, onAdd }) {
  const [channel, setChannel] = $s("65 (다음 빈 슬롯 자동 할당)");
  const [serial,  setSerial]  = $s("");
  const [autoCalib, setAutoCalib] = $s(true);

  const footer = (
    <>
      <a style={{ font: "700 11px/1 var(--font-kr)", letterSpacing: ".02em", color: "var(--content-emphasis)", textDecoration: "underline", cursor: "pointer", marginRight: "auto" }}>
        여러 채널 일괄 등록 / 관리 → [4-3] 탐촉자 설정
      </a>
      <window.Button variant="subtle"  size="sm" onClick={onClose}>취소</window.Button>
      <window.Button variant="default" size="sm" onClick={() => onAdd && onAdd(false)}>추가만</window.Button>
      <window.Button variant="emphasis" size="sm" onClick={() => onAdd && onAdd(true)}>추가 + 교정</window.Button>
    </>
  );

  return (
    <window.Modal title="센서 채널 추가 — MCF-2024-001" onClose={onClose} footer={footer}>
      ...
      <window.Toggle checked={autoCalib} onChange={setAutoCalib} size="sm" label="추가 직후 교정 마법사 자동 실행 (영점·음속·감도)"/>
    </window.Modal>
  );
};
